...
class WHCloudHelperFunctions {
  ...
  static Widget? checkSingleRecordState<WH>(AsyncSnapshot<WH> snapshot) {
  ...
  /// If an error occurs, it returns a generic error message.
  /// Otherwise, it returns null.
  static Widget? checkMultiRecordState<WH>({required AsyncSnapshot<List<WH>> snapshot, Widget? loader, Widget? error, Widget? nothingFound}) {
    if (snapshot.connectionState == ConnectionState.waiting) {
