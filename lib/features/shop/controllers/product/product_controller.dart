...
class ProductController extends GetxController {
  ...
  String getProductPrice(ProductModel product) {
    ...
    // if no variations exist return the price or saleprice
    if(product.productType == ProductType.single.toString()) {
      return (product.salePrice > 0 ? product.salePrice : product.price).toString();
    } else {
      // Calculate the smallest and largest prices among the variations
      for(var variation in product.productVariations!) {
        // Determines the price to consider (sale price if available, otherwise regular price)
      ...
      // If the smallest and largest prices are the same, return only one price
